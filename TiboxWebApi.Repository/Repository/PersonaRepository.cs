...
namespace TiboxWebApi.Repository.Repository
{
    public class PersonaRepository : BaseRepository<Persona>, IPersonaRepository
    {
        ...
        public IEnumerable<Persona> DatosDocumentacion(int nIdFlujoMaestro)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                var parameters = new DynamicParameters();
                parameters.Add("@nIdFlujoMaestro", nIdFlujoMaestro);
                return connection.Query<Persona>("WebApi_DatosPersonaDocumentacion_SP", parameters, commandType: CommandType.StoredProcedure);
            }
        }

        public IEnumerable<Persona> ElectroDatosPersona(string cDocumento)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                var parameters = new DynamicParameters();
                parameters.Add("@cNroDoc", cDocumento);
                return connection.Query<Persona>("WebApi_ElectroDatosPersona_SP", parameters, commandType: CommandType.StoredProcedure);
            }
        }

        public int ElectroInsPersona(Persona persona)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                var parameters = new DynamicParameters();
                ...
                parameters.Add("@cDirEmpleo", persona.cDirEmpleo);
                ...
                parameters.Add("@cUsuReg", persona.cUsuReg);
                ...
                parameters.Add("@cDirReferencia", persona.cDirReferencia);
                parameters.Add("@nCodPers", dbType: DbType.Int32, direction: ParameterDirection.Output);

                connection.Query<int>("WebApi_ElectroPersona_SP", parameters, commandType: CommandType.StoredProcedure);

                var resultado = parameters.Get<int>("@nCodPers");
