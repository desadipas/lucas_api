...
namespace TiboxWebApi.Repository.Repository
{
    public class CreditoRepository : BaseRepository<Credito>, ICreditoRepository
    {
        ...
        public IEnumerable<Credito> ElectroBandejaCredito(int nCodPersReg, string cTitular, string cDocumento, int nCodAge, int nNumPag, int nTamPag)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                var parameters = new DynamicParameters();
                parameters.Add("@nCodPers", nCodPersReg);
                parameters.Add("@cTitular", cTitular);
                parameters.Add("@cNro", cDocumento);
                parameters.Add("@nCodAge", nCodAge);
                parameters.Add("@nNumPagina", nNumPag);
                parameters.Add("@nTamPagina", nTamPag);

                return connection.Query<Credito>("WebApi_ElectroBandejaCredito_SP", 
                    parameters, 
                    commandType: CommandType.StoredProcedure);
            }
        }

        public int ElectroInsCredito(Credito credito)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                connection.Open();
                using (var transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        parameters.Add("@nRetorno", credito.nCodCred, dbType: DbType.Int32, direction: ParameterDirection.Output);

                        connection.Query<int>("WebApi_ElectroInsCredito_SP", parameters, commandType: CommandType.StoredProcedure, transaction: transaction);
                        var nCodCred = parameters.Get<int>("@nRetorno");
