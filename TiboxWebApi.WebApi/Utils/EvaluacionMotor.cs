...
namespace TiboxWebApi.WebApi.Utils
{
    public class EvaluacionMotor
    {
        ...
        public bool EvaluacionLucas(string cDocumento, 
                                    string cDistrito, 
                                    string cProvincia, 
        ...
        public bool EvaluacionElectro(string cDocumento,
                            string cDistrito,
                            string cProvincia,
                            string cDepartamento,
                            int nEdad,
                            ...
                            ref int nPEP)
        {
            ...
            ELScoreLenddoDatos oLenddo = new ELScoreLenddoDatos();
            ELScoreLenddoResultado oLenddoResultado = new ELScoreLenddoResultado();

            try
            {
                wsScoreLenddo.ELDocumento oLenddoDoc = new wsScoreLenddo.ELDocumento();
                wsScoreLenddo.ELProducto oLenddoPro = new wsScoreLenddo.ELProducto();

                oLenddoDoc.sNroDoc = cDocumento;
                oLenddoDoc.nTipoDoc = nTipoDocumento.ToString();
                oLenddo.oDocumento = oLenddoDoc;

                oLenddoPro.nProducto = "7";
                oLenddoPro.nModalidad = "1";
                oLenddo.oProducto = oLenddoPro;

                oLenddoResultado = oScoreLenddo.DevuelveScoreLenddo(oLenddo);

                if (oLenddoResultado.bError == true)
                {
                    cMensajeError = "ScoreLenddo Error: " + oLenddoResultado.sMensajeError;
                    return false;
                }
            }
            catch (Exception ex)
            {
                cMensajeError = "ScoreLenddo Error: " + ex.Message;
                return false;
            }
        ...
        public bool preEvaluacion(string cDocumento, int nProducto, int nModalidad, ref string cResultado, ref string cMensajeError)
        {
            try
            {
                ...
                {
                    cResultado = oPreAprobacionResultado.cDecision + "|" + oPreAprobacionResultado.cTipoScoring + "|" + oPreAprobacionResultado.nPrestamoMaximo + "|" + oPreAprobacionResultado.nPrestamoMinimo + "|" + oPreAprobacionResultado.cDescripcionRechazo;
                }
            }
            catch (Exception ex)
            {
