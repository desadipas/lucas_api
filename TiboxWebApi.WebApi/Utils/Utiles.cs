...
namespace TiboxWebApi.WebApi.Utils
{
    public class Utiles
    {
        ...
        //mdipas 25.10.2017
        public double recuperarTcea(double nPrestamo, int nCuotas, int nPeriodicidad, double nTasa, string dFechaSistema, double nSeguro)
        {
           List<Calendario> listcalendario = GeneraCalendario(nPrestamo, nCuotas, nPeriodicidad, nTasa, dFechaSistema, nSeguro);
           List<Calendario> listcalendario2 = new List<Calendario>();
           Calendario obj = new Calendario();
           obj.MontoCuota = Convert.ToString(nPrestamo * -1);
           obj.cFechaPago = dFechaSistema;
           obj.FechaPago = DateTime.Parse(dFechaSistema);
           listcalendario2.Add(obj);
        
            for (var i = 0; i < listcalendario.Count ; i++)
            {   
                Calendario nuevo = new Calendario();
                nuevo.MontoCuota = listcalendario[i].MontoCuota;
                nuevo.cFechaPago = listcalendario[i].cFechaPago;
                nuevo.FechaPago = listcalendario[i].FechaPago;
                listcalendario2.Add(nuevo);
            }
            return calcularTCEA(listcalendario2);
        }

        public double calcularTCEA( List<Calendario> calendario )
        {
            float irr = float.Parse("0.1");
            ...
            return  Math.Round( (irr * 100.00), 2 );
        }

        public float hallapvPRev( List<Calendario> calendario )
        {
            float suma = 0;
            float amt;
            for (var index = 0; index < calendario.Count; index++)
            {
                amt = float.Parse( calendario[index].MontoCuota );
                suma = suma + amt;
            }
            return suma;
        }

        public float hallaPV( float irr, int dias, List<Calendario> calendario)
        {
            float suma = 0;
            var amt = 0.0;
            float exponente = 0;
            ...
            for (var i = 0; i < calendario.Count; i++)
            {
                amt = double.Parse( calendario[i].MontoCuota );
                bases = float.Parse( "1.00" ) + irr;
                fecha =  Convert.ToString( hallaRestaFecha(calendario[i].FechaPago, calendario[0].FechaPago) / dias );
                exponente = float.Parse(fecha) ;
                potencia = Math.Pow( bases, exponente ) ;
                divi = (amt / potencia).ToString("0.######");
                suma = suma + float.Parse(divi);
            }
            return suma;
        }

        public float hallaRestaFecha(DateTime fin, DateTime inicio)
        {
            float cuantosDiasPrestado;
            TimeSpan diferencia;
            diferencia = fin.Date - inicio.Date;
            cuantosDiasPrestado = diferencia.Days;
            return cuantosDiasPrestado;
        }
    }
}