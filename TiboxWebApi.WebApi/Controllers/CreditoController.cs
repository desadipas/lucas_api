...
namespace TiboxWebApi.WebApi.Controllers
{
    ...
    public class CreditoController : BaseController
    {
        ...
        /*Electro*/
        [Route("ElectroBandejaCredito")]
        [HttpPost]
        public IHttpActionResult ElectroBandejaCredito(Credito credito)
        {
            return Ok(_unit.Credito.ElectroBandejaCredito(credito.nCodPersReg, credito.cCliente, credito.cDocumento,credito.nCodAge,credito.nNumPag, credito.nTamPag));
        }

        [Route("ElectroCredito")]
        [HttpPost]
        public IHttpActionResult ElectroCredito(Credito credito)
        {
            if (credito == null) return BadRequest();
            var nCodCred = _unit.Credito.ElectroInsCredito(credito);
            if (nCodCred == 0) return BadRequest();
            return Ok(new { nCodCred = nCodCred });
        }
        //mdipas 25.10.2017
        [Route("TCEA")]
        [HttpPost]
        public IHttpActionResult RecuperaTcea(Credito credito)
        {
            return Ok(_utils.recuperarTcea(credito.nPrestamo, credito.nNroCuotas, credito.nPeriodo, credito.nTasa, credito.dFechaSistema, credito.nSeguro));
        }
    }
}
