...
namespace TiboxWebApi.WebApi.Controllers
{
    ...
    public class PersonaController : BaseController
    {
        ...
        private EvaluacionMotor _evaluacion;
        ...
        /*Electro*/
        [Route("ElectroDatosPersona/{cDocumento}")]
        [HttpGet]
        public IHttpActionResult ElectroDatosPersona(string cDocumento)
        {
            return Ok(_unit.Persona.ElectroDatosPersona(cDocumento));
        }

        [Route("ElectroInsPersona")]
        [HttpPost]
        public IHttpActionResult ElectroInsPersonaYMotor(Persona persona)
        {
            if (persona.cTelefono == null)
            {
                persona.cTelefono = "";
            }

            var nCodPers = _unit.Persona.ElectroInsPersona(persona);

            return Ok(new { nPersona = nCodPers });
        }

        [Route("DocumentacionDatos/{id}")]
        [HttpGet]
        public IHttpActionResult DocumentacionDatos(int id)
        {
            return Ok(_unit.Persona.DatosDocumentacion(id));
        }
    }
}
